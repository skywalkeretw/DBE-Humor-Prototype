...
def isSarcastic(prediction, sentence):
    ...
    if len(prediction) > 1:
        ...
        if prediction > 0.5:
            ret = "\n" + sentence[0] + " -> is Funny: " + str(prediction[0]*100) + "%"
        else:
            ret = "\n" + sentence[0] + " -> is Not Funny: " + str(prediction[0]*100) + "%"
...
@app.route("/")
def hello_world():
    sentence = ["My IQ test results came back. They were negative."]
    sequences = tokenizer.texts_to_sequences(sentence)
    padded = pad_sequences(sequences, maxlen=max_length, padding=padding_type, truncating=trunc_type)
    isFunny = isSarcastic(model.predict(padded), sentence)
    return isFunny
